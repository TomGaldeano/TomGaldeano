...
--    Orden: cliente ASC, y a igualdad de cliente por pedido ASC.
	use tienda_online;
	SELECT nombre,id_pedido from clientes join pedidos on clientes.id_cliente = pedidos.id_cliente order by nombre asc, id_pedido asc;
-- 2) Listar cada línea de detalle con el nombre del producto y el id del pedido.
--    Columnas EXACTAS y alias:
--      - producto (pr.nombre)
--      - pedido   (dp.id_pedido)
	select productos.nombre as producto, id_pedido as pedido from productos join detalle_pedido on productos.id_producto = detalle_pedido.id_pedido order by producto asc, pedido asc;
...
select clientes.nombre as cliente, pedidos.id_pedido as pedido, pedidos.coste_total 
from clientes join pedidos on clientes.id_cliente=pedidos.id_cliente order by pedidos.coste_total desc, pedidos.id_pedido asc; 
...
SELECT pedidos.id_pedido as pedido, clientes.nombre as cliente, pedidos.fecha_pedido from clientes join pedidos on clientes.id_cliente=pedidos.id_cliente
WHERE pedidos.fecha_pedido >= '2024-01-01 00:00:00' order by pedidos.fecha_pedido asc, pedido asc;
...
SELECT pedidos.id_pedido as pedido, clientes.nombre as cliente, pedidos.estado as estado, pedidos.coste_total as coste_total from clientes join pedidos on clientes.id_cliente=pedidos.id_cliente where pedidos.estado = "cancelado" or pedidos.estado ="pendiente"
...
--    Orden: pedido ASC.
SELECT pedidos.id_pedido as pedido, clientes.nombre as cliente, pagos.metodo_pago as metodo_pago from clientes join pedidos on clientes.id_cliente 
INNER JOIN pagos on pedidos.id_pedido=pagos.id_pedido order by pedidos.id_pedido asc;
...
SELECT clientes.nombre as nombre, count(pedidos.id_pedido) as total_pedidos from clientes join pedidos on clientes.id_cliente=pedidos.id_cliente 
group by clientes.id_cliente, clientes.nombre order by count(pedidos.id_pedido) desc, clientes.nombre asc;
